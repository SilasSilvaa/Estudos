public class SystemIntegration {
    public static void main(String[] args) {
        String[] arr = { "4668c219-296d-40de-a073-99b85026e977,01-01 Visão geral do capítulo.mp4,222",
        ...
        String result = csvToJson(arr);
        System.out.println(result);

    }

    private static String csvToJson(String[] arr) {
        StringBuilder newS = new StringBuilder();

        for (int i = 0; i < arr.length; i++) {
            String id = arr[i].split(",")[0];
            String title = arr[i].split(",")[1].replaceAll("^.{6}|.{4}$", "");
            int duration = Integer.parseInt(arr[i].split(",")[2]);

            var video = new Video(id, title, duration);
            newS.append(video.toString() + ",");
        }

        return "[\n" + newS.toString().substring(0, newS.length() - 2) + "\n]";
    }
